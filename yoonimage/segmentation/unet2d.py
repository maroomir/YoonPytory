...
class Convolution2D(Module):
    def __init__(self,
        ...
        self.network = torch.nn.Sequential(
            ...
            torch.nn.BatchNorm2d(nDimOutput),
            torch.nn.ReLU(),
...
class UpSampler2D(Module):
    def __init__(self,
                 nDimInput: int,
                 nDimOutput: int):
        super(UpSampler2D, self).__init__()
        self.network = torch.nn.Sequential(
            torch.nn.Conv2d(nDimInput, nDimOutput, kernel_size=2, stride=2, bias=True),
            torch.nn.BatchNorm2d(nDimOutput),
            torch.nn.ReLU()
        )
...
class UNet2D(Module):
    def __init__(self,
        ...
        self.encoders = torch.nn.ModuleList([Convolution2D(nDimInput, nChannel, dRateDropout)])
        for i in range(nCountDepth - 1):
            self.encoders += [Convolution2D(nChannel, nChannel * 2, dRateDropout)]
            nChannel *= 2
        self.down_sampler = torch.nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
        self.worker = Convolution2D(nChannel, nChannel * 2, dRateDropout)
        self.decoders = torch.nn.ModuleList()
        self.up_samplers = torch.nn.ModuleList()
        for i in range(nCountDepth - 1):
            self.up_samplers += [UpSampler2D(nChannel * 2, nChannel)]
            self.decoders += [Convolution2D(nChannel * 2, nChannel, dRateDropout)]
            nChannel //= 2
        self.up_samplers += [UpSampler2D(nChannel * 2, nChannel)]
        self.decoders += [
            torch.nn.Sequential(
                Convolution2D(nChannel * 2, nChannel, dRateDropout),
